# -*- coding: utf-8 -*-
"""
Created on Sun Dec 15 10:51:37 2024
...
shapePredictor = "C:\\mini project\\shape_predictor_68_face_landmarks.dat"
...
arduino = serial.Serial('COM7', 9600, timeout=1)  # Adjust 'COM3' to match your port
...
ear_values = []    # Store EAR values for plotting
mar_values = []    # Store MAR values for plotting
...
while True:
    ...
    if len(rects) == 0:
        ...
        arduino.write(b'A')  # Signal Arduino to trigger alarm
    else:
        for rect in rects:
    ...
    if key == ord("q"):
        arduino.write(b'S')  # Send termination signal to Arduino
        break
...
if len(predictions) > 0:
   ...
   # Classification report for precision, recall, F1 score
    ...
    plt.axhline(y=earThresh, color='r', linestyle='--', label="EAR Threshold")
    plt.legend()
    plt.title("Eye Aspect Ratio (EAR) over Time")
    
    plt.subplot(1, 2, 2)
    plt.plot(mar_values, label="MAR",color="blue")
    plt.axhline(y=marThresh, color='r', linestyle='--', label="MAR Threshold")
    plt.legend()
